...
# Initialize persistent session states safely at startup
...
with col_step:
    if st.button("⏭ Advance 2 min", use_container_width=True):
        ...
        # Clear status banner on timeline advance so old actions don't confuse operators
        st.session_state.last_decision = None 
with col_auto:
...
# Handle state assignments on interactive click triggers
if accept_clicked:
    log_decision("ACCEPTED")
    st.session_state.last_decision = "ACCEPTED"
    st.session_state.last_decision_time = f"{latest['t_min']:.2f}"
if reject_clicked:
    log_decision("REJECTED")
    st.session_state.last_decision = "REJECTED"
    st.session_state.last_decision_time = f"{latest['t_min']:.2f}"

# Render the sticky operational feedback banner across data refreshes
if st.session_state.last_decision == "ACCEPTED":
    st.success(f"⚙️ **Recommendation accepted** at minute **{st.session_state.last_decision_time}** — "
               f"logged to the audit trail for operator handoff.")
elif st.session_state.last_decision == "REJECTED":
    st.info(f"🚫 **Recommendation rejected** at minute **{st.session_state.last_decision_time}** — "
            f"logged to the audit trail for future model-accuracy review.")
...
with left:
    ...
    st.caption(
        ...
        f"reduction in stabilization time."
    )
